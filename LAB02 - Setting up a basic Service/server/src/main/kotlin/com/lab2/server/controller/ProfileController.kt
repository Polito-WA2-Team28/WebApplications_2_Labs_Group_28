...
class ProfileController @Autowired constructor(val profileService: ProfileServiceImpl) {
    ...
    fun addProfile(@RequestBody @Validated(ProfileFormPOST::class) profile:ProfileForm, br:BindingResult){
        if(br.hasErrors()){
    ...
    fun editProfile(
        ...
        @RequestBody @Validated(ProfileFormPUT::class) profile: ProfileForm,
        bindingResult: BindingResult
    ) {

        /* Checking validation errors */
        if (bindingResult.hasErrors()) {
            val invalidFields = bindingResult.fieldErrors.map { it.field }
            throw Exception.ValidationException("", invalidFields)
